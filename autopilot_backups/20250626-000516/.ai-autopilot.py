...
class AIAutopilot(FileSystemEventHandler):
    ...
    def run_health_check(self):
        ...
        checks = [
            ...
            ("权限审计", "python modules/permission_manager.py --audit --config ./config/permission_config.yaml"),
            ("审计日志检查", "python modules/audit_logger.py --validate --config ./config/audit_config.yaml")
        ]
...
if __name__ == "__main__":
    ...
    print(f"运行状态: 监控目录 {monitor_dir}")
    backup_dir = os.path.join(monitor_dir, 'autopilot_backups')
    print(f"备份目录: {backup_dir}")
    
    # 确保所有路径操作使用绝对路径
    def get_abs_path(rel_path):
        return os.path.abspath(os.path.join(monitor_dir, rel_path))
    
    # 检查建议文件是否存在
    suggested_files = [
        os.path.join(monitor_dir, '.knowledge-mgr.py'),
        os.path.join(monitor_dir, 'modules', 'permission_manager.py')
    ]
    
    missing_files = [f for f in suggested_files if not os.path.exists(f)]
    if missing_files:
        print("提示: 以下功能文件缺失(程序仍可运行):")
        for file in missing_files:
            print(f"  - {file}")
        print("如需完整功能，请创建上述文件")
